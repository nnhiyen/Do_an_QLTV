...
public class DSChiTietPhieuNhap {
    ...
    public boolean kiemTraCTPN(String maPN, String tenFile) {
        docDuLieuTuFile(tenFile); // file phiếu nhập
    
        for (int i = 0; i < soLuongCTPN; i++) {
            if (dsChiTiet[i].getMaPN().equals(maPN)) { // phiếu nhập
                return true; 
            }
        }
    ...
    if (kiemTraCTPN(ctpn.getMaPN(), "chitietphieunhap.txt")) {
        ...
        ctpn.nhap();  // Gọi lại phương thức nhập nếu mã phiếu nhập đã tồn tại
    }

    
    if (!dsTL.kiemTraMaTL(ctpn.getMaTL())) {
        System.out.println("Mã tài liệu không hợp lệ. Không thể thêm chi tiết  phiếu nhập.");
        return; 
    }
