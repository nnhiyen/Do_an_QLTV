...
public class DSChiTietPhieuNhap {
    ...
    DsPhieuNhap dsPN= new DsPhieuNhap(100);
    ...
    if (!dsTL.kiemTraMaTL(ctpn.getMaTL())) {
        System.out.println("Mã tài liệu không tồn tại. Không thể thêm chi tiết phiếu nhập.");
        return; 
    }
    if (!dsPN.kiemTraMaPN(ctpn.getMaPN())) {
        System.out.println("Mã phiếu nhập không tồn tại. Không thể thêm chi tiết phiếu nhập.");
        return; 
    }
    ...
    if (soLuongCTPN < dsChiTiet.length) {
        ...
        soLuongCTPN++;
        ghiDuLieuRaFile("chitietphieunhap.txt");  // Ghi vào file chỉ khi mọi thứ hợp lệ
    } else {
    ...
    for (int i = 0; i < soLuongCTPN; i++) {
        if (dsChiTiet[i].isDeleted()) {
            hasDeleted = true;
            System.out.printf("| %-13s | %-11s | %-8d | %-10f | %-12f |\n",
                    dsChiTiet[i].getMaPN(),
                    dsChiTiet[i].getMaTL(),
    ...
    System.out.println("Kết quả tìm kiếm chi tiết phiếu nhập với mã: " + maPN);
    System.out.println("+--------------------------------------------------------------------+");
    ...
    for (int i = 0; i < soLuongCTPN; i++) {
        if (dsChiTiet[i].getMaPN().equals(maPN) && !dsChiTiet[i].isDeleted()) {
            System.out.printf("| %-13s | %-11s | %-8d | %-10f | %-12f |\n",
                    dsChiTiet[i].getMaPN(),
                    dsChiTiet[i].getMaTL(),
                    dsChiTiet[i].getSoLuong(),
    ...
    if (!found) {
        System.out.println("|                  Không tìm thấy chi tiết nào                   |");
    }
    System.out.println("+--------------------------------------------------------------------+");
}
...
//        double tongTien = 0;
//        for (int i = 0; i < soLuongCTPN; i++) {
//            if (!dsChiTiet[i].isDeleted()) { // Chỉ tính các chi tiết phiếu nhập chưa bị xóa
//                tongTien += dsChiTiet[i].getThanhTien(); // Thêm thành tiền của từng chi tiết vào tổng tiền
//            }
//        }
//        return tongTien;
//    }
