...
Scanner sc = new Scanner(System.in);
    ...
    public boolean kiemTraMaND(String maND) {
    try (BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\Admin\\Documents\\NetBeansProjects\\HelloAll\\src\\DoAn_QLTV_main\\src\\sourcefile\\nguoidung.txt"))) {
        ...
        while ((line = br.readLine()) != null) {
            if (line.contains("Mã sinh viên: " + maND + ",")) {
                return true;  
            }
        }
    } catch (IOException e) {
        System.out.println("Lỗi đọc file ND: " + e.getMessage());
    }
    return false;  // Nếu không tìm thấy mã ND
}
...
public void suaNguoiDung(String ma) {
    ...
    //BACKUP
    public void timKiemNguoiDung(String maNguoiDung) {
    boolean found = false;
    for (int i = 0; i < soLuong; i++) {
        if (dsNguoiDung[i] instanceof SinhVien) {
            SinhVien sv = (SinhVien) dsNguoiDung[i];
    ...
    if (!found) {
        System.err.println("Không tìm thấy người dùng có mã: " + maNguoiDung);
    }
}
        ...
        if (isGV) {
            ...
            for (int i = 0; i < soLuong; i++) {
                if (dsNguoiDung[i] instanceof GiangVien) {
                    ...
                    if (!gv.isDeleted()) {
                        System.out.printf("| %-13s | %-20s | %-6s | %-7d |\n", gv.getMaGV(), gv.getTen(), gv.getKhoa(), gv.getNamSinh());
                    }
                }
            }
            System.out.println("+---------------------------------------------------------+");
