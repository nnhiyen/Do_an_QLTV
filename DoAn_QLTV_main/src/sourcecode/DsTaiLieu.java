...
public class DsTaiLieu {
    ...
    try (BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\Admin\\Documents\\NetBeansProjects\\HelloAll\\src\\DoAn_QLTV_main\\src\\sourcefile\\tailieu.txt"))) {
        String line;
        while ((line = br.readLine()) != null) {
            if (line.contains("Mã tài liệu: " + maTL + ",")) {
                return true;  
            }
        }
    } catch (IOException e) {
        System.out.println("Lỗi đọc file TL: " + e.getMessage());
    }
  ...
  public void suaTaiLieu(String maTL) {
   ...
   public void hienThiDanhSachXoa() {
    ...
    for (int i = 0; i < soluongTL; i++) {
        if (dsTL[i].isDeleted()) {
            hasDeleted = true;
            System.out.printf("| %-17s | %-31s | %-16s | %-22s |\n",
                    dsTL[i].getMaTL(),
                    dsTL[i].getTenTL(),
                    dsTL[i].getTenTG(),
                    dsTL[i].getTenNXB());
