...
import java.io.IOException;
    ...
    // Đọc lại dữ liệu từ file để cập nhật danh sách thể loại mới nhất
   ...
   public void ghiDuLieuRaFile(String tenFile) {
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(duongDan))) {
        ...
        for (int i = 0; i < soluongTLoai; i++) {
            writer.write("Mã: " + dsTLoai[i].getMaTLoai() + ", Tên: " + dsTLoai[i].getTenTLoai() + ", IsDeleted: " + dsTLoai[i].isDeleted());
            writer.newLine();
        }
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (parts.length >= 3) {
                try {
                    ...
                    System.out.println("Lỗi khi xử lý dòng dữ liệu: " + line);
                    continue;
                }
            }
        }
